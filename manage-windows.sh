#!/usr/bin/env bash
# ============================================================================
#  Al Dawah fi al-Qarn Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/03-al-dawah-fi-al-qarn}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.aldawahfialqarn}"
AVD_NAME="${AVD_NAME:-al_dawah_fi_al_qarn_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-dawah-fi-al-qarn-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Dawah fi al-Qarn Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
